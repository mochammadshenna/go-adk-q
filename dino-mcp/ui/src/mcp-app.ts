...
function dinoSVG(name: string, color: string): string {
  ...
  const grad = `<defs><radialGradient id="${gid}" cx="35%" cy="30%" r="65%"><stop offset="0%" stop-color="rgba(255,255,255,0.95)"/><stop offset="60%" stop-color="rgba(255,255,255,0.65)"/><stop offset="100%" stop-color="rgba(0,0,0,0.25)"/></radialGradient></defs>`;
  const f = `url(#${gid})`;
  const shapes: Record<string, string> = {
    // T-Rex: right-facing bipedal — massive jaw-head, tiny arm nub, two thick legs, long tail
    "Tyrannosaurus Rex":
      `<path fill="${f}" d="M5,42C10,34 18,28 27,25C34,21 43,18 50,16C55,13 61,10 66,10C71,9 76,11 78,18L78,28C75,33 70,31 65,29C61,28 56,28 51,30L47,34C49,38 52,42 53,44L50,46C45,48 39,48 34,47C27,47 22,50 20,57C18,64 19,72 22,72L30,72 31,66 33,72 42,72C44,67 41,61 39,55C37,51 34,49 36,49C42,51 48,51 52,48C44,48 30,47 17,45C11,43 5,42 5,42Z"/>`,
    // Triceratops: right-facing quadruped — frill + 2 long brow-horns + 1 nose-horn, four legs
    "Triceratops":
      `<path fill="${f}" d="M4,54C8,48 13,46 19,46C25,45 32,45 40,44C47,43 53,42 58,40C62,37 63,32 60,28C57,24 52,22 54,20C56,17 60,16 64,18C68,20 68,26 65,30L62,32C67,29 73,27 70,33L65,35C68,33 71,33 70,38C67,42 62,40 58,40C58,43 60,47 62,51C58,53 53,50 51,47C46,47 40,49 34,53L30,66 22,66 22,54 18,66 10,66 10,54C8,54 6,54 4,54Z"/>
       <path fill="${f}" d="M64,18L78,6 74,14 66,18ZM58,22L72,12 68,20 60,24ZM63,30L76,24 72,32 64,34Z"/>`,
    // Stegosaurus: right-facing quadruped — arched body, distinctive triangular back-plates, spiked tail
    "Stegosaurus":
      `<path fill="${f}" d="M4,60C8,54 13,52 18,52C22,52 26,52 30,50C34,48 38,46 43,44C48,42 53,42 58,44C62,46 64,50 64,55C64,59 62,62 58,62L54,66 46,66 46,62 42,66 34,66 34,62 30,66 22,66 22,60C16,60 10,60 4,60Z"/>
       <path fill="${f}" d="M19,51L13,30 25,49ZM28,49L23,25 34,47ZM37,47L33,22 44,45ZM46,45L43,26 54,43ZM57,45L56,32 64,45Z"/>
       <path fill="${f}" d="M4,58L10,64 4,66ZM2,56L8,60 6,66Z"/>`,
    // Velociraptor: right-facing running — lean horizontal body, stiff tail, raised sickle claw
    "Velociraptor":
      `<path fill="${f}" d="M4,36C8,30 14,26 20,24C23,23 26,23 28,22L33,14C35,12 38,13 37,17L32,22C36,20 40,19 40,23C39,27 36,29 33,31L30,33C32,37 35,42 34,46L30,50C27,52 23,52 20,52C17,50 15,47 16,43C13,45 9,46 6,45Z"/>
       <path fill="${f}" d="M4,36C9,33 16,30 22,28L28,26 30,33 22,24Z"/>
       <path fill="${f}" d="M20,52L22,64 26,70C29,70 29,66 26,64L26,52Z"/>
       <path fill="${f}" d="M28,50L30,62 34,68C37,68 37,64 34,62L34,50Z"/>
       <path fill="${f}" d="M34,46L48,42 44,36Z"/>`,
    // Brachiosaurus: tall neck reaching upper-right, small head, large body, four legs
    "Brachiosaurus":
      `<path fill="${f}" d="M4,60C8,54 13,52 18,52C21,52 24,52 27,51C29,47 32,41 35,35C38,29 42,22 46,16C48,12 51,9 55,9C58,9 60,11 59,15C58,19 55,22 52,23L49,24L52,24C50,26 47,28 45,31C43,34 41,38 39,43C37,48 35,53 34,57L40,59 46,63 46,67 40,67 40,63 34,63 32,67 24,67 24,61 20,67 12,67 12,61C9,61 6,61 4,60Z"/>`,
    // Ankylosaurus: low armored quadruped — wide flat body, small head, bony spikes on back, club tail
    "Ankylosaurus":
      `<path fill="${f}" d="M6,62C2,58 2,52 6,50C8,48 12,46 16,46C14,44 14,40 18,38C22,36 28,36 32,38C36,36 42,34 48,34C54,34 60,36 64,40C68,40 72,42 74,46C78,46 80,50 78,54C76,58 72,60 68,62L60,66 52,66 52,62 48,66 40,66 40,62 36,66 28,66 28,62 24,66 16,66 16,62C12,62 8,62 6,62Z"/>
       <path fill="${f}" d="M16,46L12,34 22,44ZM25,42L22,30 32,40ZM34,40L32,27 42,38ZM43,40L42,28 52,38ZM54,40L54,28 62,40Z"/>
       <path fill="${f}" d="M74,50C78,46 82,48 82,54C82,58 78,62 74,60Z"/>`,
    // Diplodocus: extremely long neck (upper-right) AND long whip-tail (left), four legs
    "Diplodocus":
      `<path fill="${f}" d="M2,40C5,36 9,34 14,34C17,34 20,35 21,37C23,33 26,28 30,23C33,18 37,13 42,10C45,8 48,9 48,12C47,16 44,19 40,21L36,22L40,22C37,24 34,27 32,30C29,33 27,37 26,40L34,42 42,46 42,50 36,50 36,46 30,46 28,50 20,50 20,44 16,50 8,50 8,44C5,44 3,42 2,40Z"/>
       <path fill="${f}" d="M2,40C7,37 13,34 19,33L26,32 28,38 20,34Z"/>`,
    // Spinosaurus: right-facing bipedal like T-Rex but with tall neural spine sail on back
    "Spinosaurus":
      `<path fill="${f}" d="M5,46C10,38 18,32 27,29C33,26 39,24 44,22C47,20 50,18 52,20L50,28C53,22 57,18 58,22L56,30C59,24 63,20 64,24L62,32C65,26 68,22 68,26L65,34 60,36 54,34 50,36L46,38C48,42 51,46 52,48L49,50C44,52 38,52 33,51C26,51 21,54 19,61C17,68 18,75 21,75L29,75 30,69 32,75 41,75C43,70 40,64 38,58C36,54 33,52 35,52C41,54 47,54 51,51C43,51 29,50 16,48C10,47 5,46 5,46Z"/>`,
    // Parasaurolophus: right-facing bipedal hadrosaur — duck bill, distinctive backward tube crest
    "Parasaurolophus":
      `<path fill="${f}" d="M4,52C8,46 13,44 19,44C25,43 32,43 39,42C45,41 51,41 56,42C60,40 63,36 65,34C67,30 68,24 66,20C64,16 60,14 57,16C54,18 53,22 55,26C52,24 50,22 48,26C50,30 52,36 54,40L50,44C48,42 46,42 44,44L40,58 32,58 32,50 28,60 20,60 20,52 16,62 8,62 8,54C6,54 5,53 4,52Z"/>
       <path fill="${f}" d="M66,20C68,16 72,11 76,9C79,8 80,11 78,15C76,19 71,24 66,26Z"/>`,
    // Pachycephalosaurus: right-facing bipedal — very prominent bone dome on top of skull
    "Pachycephalosaurus":
      `<path fill="${f}" d="M4,52C8,46 13,44 19,44C25,43 31,43 37,44L43,46C45,42 47,37 48,32C49,27 49,22 47,18C45,14 41,12 38,14C35,16 34,20 35,26C32,22 29,20 28,24C28,28 30,32 33,36L37,44C39,48 40,52 39,56L35,64 27,64 27,56 24,64 16,64 16,56 12,64 4,64 8,56C7,54 5,53 4,52Z"/>
       <path fill="${f}" d="M35,26C35,20 37,14 41,12C46,10 50,14 49,20C48,26 44,30 38,32Z"/>`,
    // Raptor (Deinonychus): right-facing bipedal — larger than Velociraptor, prominent sickle claw raised
    "Raptor":
      `<path fill="${f}" d="M4,40C8,33 15,28 22,26C25,25 28,25 30,23L35,15C37,13 40,14 39,18L34,23C38,21 42,20 42,24C41,28 38,30 35,32L32,34C34,38 37,43 36,47L31,52C28,54 24,54 20,54C17,52 15,49 16,45C13,47 9,48 6,47Z"/>
       <path fill="${f}" d="M4,40C9,37 16,33 22,31L30,29 32,36 24,28Z"/>
       <path fill="${f}" d="M21,54L23,66 27,72C30,72 30,68 27,66L27,54Z"/>
       <path fill="${f}" d="M29,52L31,64 35,70C38,70 38,66 35,64L35,52Z"/>
       <path fill="${f}" d="M36,47L50,43 46,37Z"/>`,
    // Maiasaura: right-facing hadrosaur — large duck-billed head, broad body, no crest (distinguishes from Parasaurolophus)
    "Maiasaura":
      `<path fill="${f}" d="M4,54C8,48 13,46 19,46C25,45 32,45 40,44C46,43 52,43 57,44C61,44 64,42 66,40C68,38 70,34 68,30C66,26 62,24 58,26C55,28 54,32 57,36C53,34 50,32 50,36C50,40 53,44 57,46L52,48C48,46 44,46 40,48L36,62 28,62 28,54 24,64 16,64 16,56 12,64 4,64 8,56C7,54 5,54 4,54Z"/>
       <path fill="${f}" d="M66,40C68,36 72,32 76,32C78,32 79,35 77,38C75,41 71,44 68,44Z"/>`,
  };
  const body = shapes[name] ?? `<ellipse cx="40" cy="50" rx="28" ry="18" fill="${f}"/>`;
